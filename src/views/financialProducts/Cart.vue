<template>
      ...
      <v-card v-else>
        ...
        <v-card-text>
          <v-row class="header-row">
            ...
            <v-col class="text-h6"></v-col> <!-- For the delete button -->
          </v-row>
          <v-divider></v-divider>
          ...
          <v-pagination v-model="currentPage" :length="totalPages" @input="updatePagination" class="mt-4"></v-pagination>
        </v-card-text>
  
        <div class="Cart-Btn-Set mt-4">
          <v-btn class="cart-Btn-Gotocompare" @click="goToCompare" color="primary">상품 비교해보기</v-btn>
          <v-btn class="cart-Btn-GotoPortfolio" @click="goToMakePortfolio" color="secondary">포트폴리오 구성하기</v-btn>
        </div>
      </v-card>
    </div>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const totalPages = computed(() => Math.ceil(cart.value.length / itemsPerPage)); // 총 페이지 수 계산
      ...
      const goToCompare = () => {
        router.push('/product-comparison'); // 상품 비교 페이지로 이동
      };
