      ...
      <section class="portfolio-summary">
        ...
        <v-card class="mt-8 mx-auto overflow-visible" max-width="400">
          <v-sheet
            ...
            rounded="lg"
          >
            <!-- v-sparkline을 사용하여 투자 경과를 선 그래프로 표현 -->
            <v-sparkline
              :labels="labels"
...
import { ref, computed, onMounted, onBeforeUnmount } from 'vue'; // Vue 3 Composition API 사용

export default {
  name: 'Portfolio',
  components: {
    apexchart: VueApexCharts, // ApexCharts 컴포넌트 등록
  },
  setup() {
    const route = useRoute(); // 현재 라우트에서 포트폴리오 ID를 가져옴
    ...
    // 포트폴리오 데이터 상태 관리
    const portfolio = ref(fetchPortfolioData(portfolioId)); // 포트폴리오 데이터 저장
